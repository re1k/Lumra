...
class CalendarController extends GetxController {                         
  ...
  final visibleMonth = monthStart(DateTime.now()).obs; //the current month is the month that is going to appear                  
  final selectedDay = Rxn<DateTime>();                                    
  final monthEvents = <DateTime, List<CalendarEvent>>{}.obs; //contains each day with the list of events assigned to that day             
  ...
  Future<void> _watchMonth(DateTime m) async {                            
    ...
    final q = db                                                           
        ...
        .where('start', isGreaterThanOrEqualTo: Timestamp.fromDate(start)) //to ensure they are events of the selected month
